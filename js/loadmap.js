...
//    });
    ...
    function initMap() {
        ...
        getAjaxData(map, "data/geotweets_merc3857.geo.json", "tweets", "layerOff", gmarkersTweets);
        getAjaxData(map, "data/attractions_4326.geo.json", "attractions", "layerOn", gmarkersAttractions);
    ...
    */
    function getAjaxData(map, dataUrl, category, showOption, markerList) {
        ...
        // show or hide the categories initially with showOption
        if (showOption == "layerOn") {
            toggleLayer(category, markerList, true);
        } else if (showOption == "layerOff") {
    ...
    */
    function parsePoints(data, category, markerList) {
        for (var i = 0; i < data.features.length; i++) {
            ...
            var marker = createMarker(pointLatLong, name, html, category, markerList) //,category);
            // category could be used if we want to categorize the types of tweets
    //                var category = item.cat;
        };
        addToggleButtons(category, markerList);
    };
    ...
    @param category: string of type of data or layers from different geojson files
    */
    function createMarker(latlng, name, html, category, markerList) {
...
//        $("#"+category+"box").checked = true;  
        ...
        $("#"+category+"box").prop("checked", toggleOption);
//        document.getElementById(category+"box").checked = toggleOption; 
        console.log("checkbox:", $("#"+category+"box").prop("checked", toggleOption))
